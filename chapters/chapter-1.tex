...
meningkatkan akurasi diagnosis, personalisasi pengobatan, serta membuat 
prosesadministrasi di rumah sakit menjadi efisien \cite{info:doi/10.2196/53616}. 
Kemajuan teknologi kesehatan digital telah mendorong transformasi sistem 
pelayanan medis menuju pendekatan yang lebih terintegrasi dan berbasis 
informasi. Meski demikian, aspek keselamatan data pribadi tetap menjadi 
fokus utamanya \cite{lee2024status}. Melalui transformasi tersebut, 
diharapkan terjadi perluasan akses masyarakat terhadap pelayanan 
kesehatan yang berkualitas dan terjangkau. \par

Salah satu implementasi AI yang menonjol dalam bidang kesehatan adalah
pada pengolahan citra medis. Kontribusi AI dalam sektor kesehatan 
mengalami perkembangan signifikan, khususnya pada citra medis dan 
klasifikasi penyakit. Dengan menggunakan \textit{deep learning} seperti
\textit{Convolutional Neural Network} (CNN) menunjukkan performa yang 
sangat baik dalam mengidentifikasi dan mengelompokkan berbagai penyakit 
dari citra medis seperti fundus mata, radiologi, dan hasil pemindaian 
lainnya. Pendekatan ini memungkinkan ekstraksi fitur penting secara 
otomatis, sehingga mempercepat proses diagnosis dan memperkecil risiko 
kesalahan manusia dalam interpretasi gambar medis \cite{Nozomi_Aldi_Sentosa_2022}. 
Hasil penelitian dari berbagai studi literatur mengindikasikan bahwa AI 
berkontribusi dalam meningkatkan akurasi deteksi penyakit serta 
menyediakan alternatif otomatisasi untuk analisis citra medis yang 
sebelumnya sangat bergantung pada keahlian manual \cite{li2023medical}. 
Selain itu, pengembangan framework inovatif berbasis \textit{deep learning} 
berperan besar dalam mengintegrasikan patologi dan radiologi, sehingga 
menghasilkan klasifikasi penyakit yang lebih presisi dan transparan \cite{he2025deep}. 
AI dengan tingkat presisi yang menyamai atau bahkan mengungguli kemampuan
praktisi medis, berkontribusi dalam mengoptimalkan manajemen perawatan
pasien melalui pendekatan yang lebih fokus. \par

Dalam penerapannya pada klasifikasi penyakit spesifik seperti penyakit 
kulit, muncul tantangan terkait ketidakseimbangan data. 
Klasifikasi penyakit kulit menggunakan citra medis memiliki salah satu 
tantangan yaitu data yang tidak seimbang 
antar kelas penyakit. Masalah ini menyebabkan model menghasilkan prediksi 
yang lebih akurat pada kelas mayoritas dibandingkan dengan kelas minoritas 
yang performanya lebih rendah \cite{nurkhasanah2021klasifikasi}. 
Ketidakseimbangan data dapat memicu bias prediksi dan sensitivitas model 
akan berkurang terhadap penyakit minoritas, yang akhirnya menyebabkan 
risiko misdiagnosis pada kasus penting meningkat. Pada penelitian 
klasifikasi penyakit kulit wajah menggunakan ResNet-50, ditemukan bahwa 
data yang tidak terdistribusi dengan seimbang menyebabkan presisi dan 
recall menjadi rendah. \cite{khani2025penerapan}.
...
antar kelas. Dalam mengatasi masalah tersebut, teknik augmentasi data 
dapat dimanfaatkan pada kelas minoritas supaya performa model meningkat \cite{prananda2024klasifikasikan}. \par
...
tidak seimbang pada klasifikasi penyakit kulit berbasis citra medis, 
terutama dalam mendeteksi penyakit yang spesifik atau langka \cite{chazar2025pendekatan}. 
Melalui teknik augmentasi, jumlah dan keberagaman 
data pada kelas minoritas dapat ditingkatkan sehingga model memiliki 
peluang lebih besar untuk mempelajari karakteristik kelas tersebut secara 
lebih representatif. Pemilihan arsitektur deep learning yang tepat 
merupakan langkah penting. Berbagai arsitektur deep learning seperti 
VGG, DenseNet, EfficientNet, dan ResNet telah banyak diterapkan dalam 
klasifikasi citra medis dengan masing-masing karakteristik dan keunggulan 
yang berbeda \cite{Berliani_Rahardja_Septiana_2023} \cite{Ade_Maulani_Bilgis_Nurhadi_Surojudin_Ahmad_Fauzi_2025} \cite{kadric2025improvingdiagnosticaccuracypigmented}. 
Di antara arsitektur tersebut, ResNet dikenal memiliki 
kemampuan yang baik dalam menangani jaringan yang lebih dalam melalui 
mekanisme residual learning, sehingga mampu menjaga stabilitas proses 
pelatihan dan mempertahankan performa model. Maka dari itu, untuk penelitian 
yang dilakukan menggunakan arsitektur ResNet. \par

Jenis arsitektur ResNet yang digunakan adalah ResNet-18 dan ResNet-50. 
Kedua jenis arsitektur menerapkan 
konsep \textit{residual learning} melalui \textit{skip connections} untuk 
mengatasi masalah vanishing dan \textit{exploding gradient}, di mana 
ResNet-50 memfasilitasi aliran informasi pada jaringan dalam, sedangkan 
ResNet-18 dibangun dengan struktur spesifik yang terdiri dari 17 lapisan 
konvolusi, max pooling $3\times3$, serta lapisan fully connected dengan 
fungsi aktivasi ReLU \cite{jiemesha2025classification} \cite{francis2025deep}. 
Performa model dievaluasi menggunakan Matthews Correlation Coefficient (MCC) 
untuk memastikan penilaian yang adil, terutama pada kasus dengan ukuran 
kelas data yang tidak seimbang \cite{aggarwal2019data}. Selain itu, 
evaluasi performa juga dilengkapi dengan penggunaan \textit{confusion matrix} 
yang meliputi akurasi, \textit{precision}, \textit{recall}, dan \textit{f1-score}. 
Hasil evaluasi dari masing-masing arsitektur yaitu ResNet-18 dan ResNet-50 
akan dibandingkan untuk menentukan arsitektur yang memberikan performa 
terbaik dalam klasifikasi citra medis sesudah melakukan augmentasi data. \par
...
dalam penelitian ini adalah sebagai berikut: \par

\begin{enumerate}
    \item Bagaimana menerapkan teknik augmentasi data yang ditujukan hanya pada kelas minoritas untuk meningkatkan hasil model klasifikasi citra penyakit kulit berbasis dataset DermaMNIST yang tidak seimbang ?
    \item Bagaimana hasil arsitektur ResNet-18 dan ResNet-50 dalam mengklasifikasikan citra penyakit kulit setelah diterapkan augmentasi khusus pada kelas minoritas dibandingkan tanpa augmentasi ?
    \item Bagaimana \textit{confusion matrix} digunakan sebagai evaluasi dasar serta pengukuran MCC dalam mengukur hasil model pada klasifikasi penyakit kulit dengan kondisi data yang tidak seimbang ?
\end{enumerate}
...
Penelitian ini bertujuan untuk mengatasi permasalahan ketidakseimbangan 
data pada klasifikasi citra penyakit kulit dengan menerapkan pendekatan 
augmentasi pada kelas minoritas. Secara khusus, tujuan dari penelitian 
...
\begin{enumerate}
    ...
    \item Mengukur hasil arsitektur ResNet-18 dan ResNet-50 dalam klasifikasi citra penyakit kulit sebelum dan sesudah penerapan augmentasi pada kelas minoritas, untuk mengetahui efektivitas strategi augmentasi terhadap akurasi dan keadilan prediksi antar kelas.
    \item Mengevaluasi hasil model dengan menggunakan \textit{confusion matrix} sebagai evaluasi dasar serta pengukuran MCC dalam mengukur hasil model klasifikasi penyakit kulit pada data yang tidak seimbang.
\end{enumerate}
...
Supaya ruang lingkup penelitian ini lebih terarah dan realistis untuk 
diselesaikan dalam waktu yang telah ditentukan, maka penelitian ini 
dibatasi pada hal-hal berikut: \par
...
Penelitian diharapkan dapat memberikan manfaat baik bagi penulis sendiri 
maupun dunia akademik. Adapun manfaat dari penelitian sebagai berikut: \par
