...
\begin{enumerate}[noitemsep]
	...
	kondisi serius seperti melanoma. Penelitian ini juga menyoroti 
	berbagai kelemahan pada dataset DermaMNIST orisinal, termasuk resolusi 
	gambar yang rendah (28x28), ketidakseimbangan data, dan organisasi 
	data yang buruk yang terbukti tidak memadai untuk klasifikasi yang 
	andal. Terdapat perbandingan metode yang digunakan sebelumnya. 
	Penelitian sebelumnya membandingkan model ResNet-50 dan 
	EfficientNetV2L, serta menerapkan \textit{transfer learning} pada dua 
	versi dataset yaitu DermaMNIST orisinal dan DermaMNIST-C (versi 
	perbaikan resolusi 224x224). Sedangkan penelitian yang akan dilakukan 
	membandingkan model ResNet-18 dan ResNet-50 \cite{kadric2025improvingdiagnosticaccuracypigmented}.
	...
	Penelitian ini menggunakan dataset DermaMNIST yang terdiri dari 7 
	kelas yang berbeda. Penelitian ini meneliti dengan menggunakan data 
	yang sedikit dengan cara seleksi instance menggunakan K-Means 
	Clustering. Namun, penelitian yang akan dilakukan tidak mengurangi 
	data melainkan menambahkan data pada kelas minoritas. Selain itu, 
	penelitian akan dilakukan menggunakan arsitektur ResNet-18 dan 
	ResNet-50 sedangkan penelitian sebelumnya menggunakan Lighweight 
	Deep Models \cite{gonzalez2025lightweightdeepmodelsdermatological}.

	\item Penelitian sebelumnya oleh Arindam Halder, Anogh Dalal, 
	Sanghita Gharami, Marcin Wozniak, Muhammad Fajal Ijaz, dan Pawan 
	Kumar Singh pada tahun 2025 dengan judul penelitian "\textit{A fuzzy 
	rank-based deep ensemble methodology for multi-class skin cancer 
	classification}" membahas masalah klasifikasi kanker kulit multi-kelas 
	dan tantangan \textit{imbalanced dataset}. Penelitian ini menggunakan 
	\textit{deep ensemble} yang menggabungkan beberapa arsitektur, yaitu 
	Xception, InceptionResNetV2, dan MobileNetV2. Dalam mengatasi data 
	yang tidak seimbang, penelitian sebelumnya menerapkan augmentasi data 
	untuk menyeimbangkan dataset sebelum proses pelatihan. Dengan 
	menerapkan augmentasi data membuktikan bahwa dapat berhasil 
	meningkatkan performa. Penelitian sebelumnya relevan dengan penelitian 
	yang akan dilakukan dengan adanya beberapa perbedaan. Penelitian yang 
	akan dilakukan menerapkan augmentasi data pada kelas minoritas serta 
	menggunakan ResNet-18 dan ResNet-50 untuk mengukur pengaruh augmentasi 
	terhadap performa kedua model \cite{halder2025fuzzy}.
	...
	dengan judul "Implementasi Algoritma \textit{Convolutional Neural 
	Network} (Resnet-50) untuk Klasifikasi Kanker Kulit Benign dan 
	Malignant". Penelitian ini membahas tentang klasifikasi kanker kulit 
	untuk membedakan antara lesi \textit{Benign} dan \textit{Malignant}, 
	yang merupakan masalah klasifikasi biner. Penelitian ini melakukan 
	implmentasi \textit{transfer learning} menggunakan arsitektur 
	ResNet-50 yang telah dilatih. Penelitian sebelumnya relevan karena 
	menggunakan arsitektur yang sama yaitu ResNet-50. Terdapat perbedaan 
	dengan penelitian yang akan dilakukan, yaitu menangani klasifikasi 
	multi-kelas dengan masalah \textit{imbalanced data}. Serta melakukan 
	augmentasi data pada kelas minoritas \cite{Gusti_Haerani_Syafria_Yanto_Gusti_2024}. 
	Dengan demikian, penelitian ini dapat dijadikan sebagai referensi awal 
	dalam memahami penerapan arsitektur ResNet pada citra medis. Selain 
	itu, hasil penelitian tersebut memberikan gambaran umum mengenai 
	performa model deep learning dalam konteks klasifikasi penyakit kulit.
	...
	strategi augmentasi otomatis (\textit{Low-Cost-Augment}) yang 
	teroptimasi untuk meningkatkan keseimbangan data dan performa 
	klasifikasi pada dataset HAM10000. Meskipun arsitektur utama yang 
	diuji berupa EfficientNet, hasil eksperimen menunjukkan bahwa 
	augmentasi otomatis dapat meningkatkan \textit{accuracy} tanpa 
	memerlukan data eksternal. Namun, keterbatasannya adalah belum diuji 
	pada arsitektur ResNet-18 dan ResNet-50 serta belum diaplikasikan pada 
	dataset DermaMNIST \cite{xuronald2022low}. Oleh karena itu, penelitian 
	ini tetap memiliki kontribusi penting dalam pengembangan strategi 
	augmentasi data pada bidang klasifikasi citra medis. Selain itu, 
	pendekatan yang diusulkan dapat menjadi acuan awal bagi penelitian 
	lanjutan dengan arsitektur dan dataset yang berbeda.
\end{enumerate}
...
% 	\hline
% 	3. & Arindam Halder, Anogh Dalal, Sanghita Gharami, Marcin Wozniak, Muhammad Fazal Ijaz, Pawar Kuman Singh [2025] [A fuzzy rank-based deep ensemble methodology for multi-class skin cancer classification] 
...
% 	\hline
% 	4. & Gogor Putra Hafi Puja Gusti, Elin Haerani, Fadhillah Syafria, Febi Yanto, Siska Kurnia Gusti [2024] [Implementasi Algoritma Convolutional Neural Network (ResNet-50) untuk Klasifikasi Kanker Kulit Benign dan Malignant] 
...
% 	\hline
% 	5. & Shuwei Shen, Mengjuan Xu, Fan Zhang, Pengfei Shao, Honghong Liu, Liang Xu, Chi Zang, Peng Liu, Peng Yao, Ronald X. Xu [2022] [A Low-Cost High-Performance Data Augmentation for Deep Learning-Based Skin Lesion Classification] 
...
%     \hline
%     \textbf{No.} & \centering \textbf{Judul} & \centering \textbf{Masalah} & \centering \textbf{Metode} & \centering \textbf{Hasil} \tabularnewline
%     \hline
%     \endfirsthead
...
%     1. & Nerma Kadric, Amila Akagic, Medina Kapo [2025] [\textit{Improving Diagnostic Accuracy of Pigmented Skin Lesions With CNNs: A Case Study with DermaMNIST Dataset}] 
% 	   & Kelemahan dataset DermaMNIST yang memiliki resolusi gambar yang rendah (28 x 28), \textit{imbalanced data}. 
% 	   & \textit{Trasnfer learning} arsitektur ResNet-50 dan EfficientNetV2L dengan menggunakan dataset DermaMNIST orisinal dan DermaMNIST-C (resolusi 224 x 224). 
% 	   & Arsitektur EfficientNetV2L mencapai akurasi 84.90\% pada dataset DermaMNIST-C. \\* \hline

%     2. & Ian Mateos Gonzales, Estefani Jaramilla Nava, Abraham Sánchez Morales, Jesús García Ramírez, Ricardo Ramos Aguilar [2025] [\textit{Lightweight Deep Models for Dermatological Disease Detection: A Study on Instance Selection and Channel Optimization}] 
% 	   & Model \textit{deep learning} yang ringan untuk deteksi penyakit dermatologis. 
% 	   & Menggunakan \textit{Lightweight Deep Models}, melakukan \textit{instance selection} menggunakan K-Means Clustering untuk mengurangi data pada kelas mayoritas. 
% 	   & Model dengan 472K parameter mencapai akurasi 71.57\%, dengan komputasi yang lebih efisien yang sebanding dengan model ResNet. \\* \hline

%     3. & Arindam Halder, Anogh Dalal, Sanghita Gharami, Marcin Wozniak, Muhammad Fazal Ijaz, Pawar Kuman Singh [2025] [\textit{A fuzzy rank-based deep ensemble methodology for multi-class skin cancer classification}] 
% 	   & \textit{Imbalanced data} pada klasifikasi kanker kulit multi-class. 
% 	   & \textit{Fuzzy ensemble} dengan menggabungkan Xception, InceptionResNet V2, dan MobileNetV2. Menerapkan augmentasi data untuk menyeimbangkan dataset. 
% 	   & Model ensemble mencapai akurasi 95\% pada dataset HAM10000 dan mencapai akurasi 73\% dengan menguji model dengan dataset DermaMNIST. \\* \hline

%     4. & Gogor Putra Hafi Puja Gusti, Elin Haerani, Fadhillah Syafria, Febi Yanto, Siska Kurnia Gusti [2024] [Implementasi Algoritma Convolutional Neural Network (ResNet-50) untuk Klasifikasi Kanker Kulit Benign dan Malignant] 
% 	   & Klasifikasi kanker kulit untuk membedakan antara Benign (jinak) dan Malignant (ganas), yang merupakan masalah klasifikasi biner. 
% 	   & Implementasi \textit{transfer learning} menggunakan ResNet-50 yang \textit{pre-trained}. 
% 	   & Dengan menggunakan ResNet-50 mencapai akurasi 90\% dengan \textit{loss} 13\%. \\* \hline

%     5. & Shuwei Shen, Mengjuan Xu, Fan Zhang, Pengfei Shao, Honghong Liu, Liang Xu, Chi Zang, Peng Liu, Peng Yao, Ronald X. Xu [2022] [\textit{A Low-Cost High-Performance Data Augmentation for Deep Learning-Based Skin Lesion Classification}] 
% 	   & Data klasifikasi lesi kulit yang tidak seimbang. 
% 	   & Menggunakan arsitektur EfficientNet b2 serta menerapkan \textit{Low Cost Augment} untuk augmentasi data. 
% 	   & Dengan menerapkan \textit{Low Cost Augment} dan menggunakan arsitektur EfficientNet b2, hasil yang didapatkan dengan evaluasi BACC mencapai 0.853. \\* \hline
% \end{tabularx}

\begin{landscape} % Memulai halaman landscape
\begin{small}      % Opsional: memperkecil sedikit font agar lebih lega

\begin{tabularx}{\linewidth}{| c | p{0.25\linewidth} | X | X | X |}
    \caption{Tabel Studi Literatur}
    \label{table:2.literasi}\\
    ...
    1. & Nerma Kadric, dkk [2025] [\textit{Improving Diagnostic Accuracy of Pigmented Skin Lesions With CNNs: A Case Study with DermaMNIST Dataset}] 
       & Kelemahan dataset DermaMNIST yang memiliki resolusi gambar yang rendah (28 x 28), \textit{imbalanced data}. 
       & \textit{Trasnfer learning} arsitektur ResNet-50 dan EfficientNetV2L dengan menggunakan dataset DermaMNIST orisinal dan DermaMNIST-C (224 x 224). 
       & Arsitektur EfficientNetV2L mencapai akurasi 84.90\% pada dataset DermaMNIST-C. \\ \hline

    2. & Ian Mateos Gonzales, dkk [2025] [\textit{Lightweight Deep Models for Dermatological Disease Detection: A Study on Instance Selection and Channel Optimization}] 
       & Model \textit{deep learning} yang ringan untuk deteksi penyakit dermatologis. 
       & Menggunakan \textit{Lightweight Deep Models}, melakukan \textit{instance selection} menggunakan K-Means Clustering. 
       & Model dengan 472K parameter mencapai akurasi 71.57\%, dengan komputasi yang lebih efisien yang sebanding dengan model ResNet. \\ \hline

    3. & Arindam Halder, dkk [2025] [\textit{A fuzzy rank-based deep ensemble methodology for multi-class skin cancer classification}] 
       & \textit{Imbalanced data} pada klasifikasi kanker kulit multi-class. 
       & \textit{Fuzzy ensemble} (Xception, InceptionResNet V2, dan MobileNetV2). Menerapkan augmentasi data. 
       & Model ensemble mencapai akurasi 95\% pada HAM10000 dan 73\% pada dataset DermaMNIST. \\ \hline

    4. & Gogor Putra Hafi P. G., dkk [2024] [Implementasi Algoritma CNN (ResNet-50) untuk Klasifikasi Kanker Kulit Benign dan Malignant] 
       & Klasifikasi kanker kulit untuk membedakan antara Benign (jinak) dan Malignant (ganas). 
       & Implementasi \textit{transfer learning} menggunakan ResNet-50 yang \textit{pre-trained}. 
       & Dengan menggunakan ResNet-50 mencapai akurasi 90\% dengan \textit{loss} 13\%. \\ \hline

    5. & Shuwei Shen, dkk [2022] [\textit{A Low-Cost High-Performance Data Augmentation for Deep Learning-Based Skin Lesion Classification}] 
       & Data klasifikasi lesi kulit yang tidak seimbang. 
       & Menggunakan arsitektur EfficientNet b2 serta menerapkan \textit{Low Cost Augment} untuk augmentasi data. 
       & Dengan menerapkan \textit{Low Cost Augment} dan EfficientNet b2, hasil evaluasi BACC mencapai 0.853. \\ \hline

\end{tabularx}

\end{small}
\end{landscape} % Kembali ke halaman portrait
...
\textit{vascular lesions} \cite{Yang_2023}. Dataset ini digunakan dalam 
penelitian klasifikasi citra medis karena memiliki standar anotasi yang 
jelas. Selain itu, ketersediaannya secara publik memudahkan proses 
replikasi dan perbandingan hasil penelitian. \par

\subsubsection{Actinic Keratosis and Intraepithelial Carcinoma} \label{II.Teori1.1}

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.4\textwidth]{figure/class_0_actinic_keratoses_and_intraepithelial_carcinoma.png}}
	\caption{Sampel Kelas \textit{Actinic Keratosis and Intraepithelial Carcinoma}}
	\label{fig:2.Class0}
\end{figure}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.5\textwidth]{figure/class_1_basal_cell_carcinoma.png}}
	\caption{Sampel Kelas \textit{Basal Cell Carcinoma}}
	\label{fig:2.Class1}
\end{figure}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.6\textwidth]{figure/class_2_benign_keratosis-like_lesions.png}}
	\caption{Sampel Kelas \textit{Benign Keratosis-Like Lesions}}
	\label{fig:2.Class2}
\end{figure}
...
ketiga jenis tersebut dapat terlihat berbeda \cite{tschandl2018ham10000}. 
Secara klinis, ketiga lesi tersebut sama-sama bersifat jinak dan tidak 
menunjukkan karakteristik keganasan. Oleh karena itu, pengelompokan BKL 
sering digunakan dalam studi dermatologi dan dataset citra medis. 
Selain itu, kesamaan pola visual pada ketiga lesi tersebut menjadikan BKL 
relevan untuk dikaji dalam penelitian. \par

\subsubsection{Dermatofibroma} \label{II.Teori1.4}

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.4\textwidth]{figure/class_3_dermatofibroma.png}}
	\caption{Sampel Kelas \textit{Dermatofibroma}}
	\label{fig:2.Class3}
\end{figure}
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.4\textwidth]{figure/class_4_melanoma.png}}
	\caption{Sampel Kelas \textit{Melanoma}}
	\label{fig:2.Class4}
\end{figure}
...
kanker dapat disembuhkan \cite{tschandl2018ham10000}. Pada stadium lanjut 
melanoma memiliki tingkat agresivitas yang tinggi dan berpotensi menyebar 
ke organ lain. Secara dermatoskopik, melanoma sering menunjukkan pola yang 
tidak teratur, variasi warna, serta batas lesi yang tidak jelas. Oleh 
karena itu, deteksi dini melanoma menjadi aspek yang sangat penting dalam 
upaya menurunkan angka mortalitas akibat kanker kulit. \par

\subsubsection{Melanocytic Nevi} \label{II.Teori1.6}

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.5\textwidth]{figure/class_5_melanocytic_nevi.png}}
	\caption{Sampel Kelas \textit{Melanocytic Nevi}}
	\label{fig:2.Class5}
\end{figure}
...
berbeda satu sama lain \cite{tschandl2018ham10000}. Perbedaan ini mencakup 
variasi warna, struktur, dan pola distribusi pigmen pada permukaan kulit. 
Secara klinis, melanocytic nevi umumnya bersifat non-malignan dan memiliki 
risiko rendah untuk berkembang menjadi melanoma. \par

\subsubsection{Vascular Lesions} \label{II.Teori1.7}

\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=0.5\textwidth]{figure/class_6_vascular_lesions.png}}
	\caption{Sampel Kelas \textit{Vascular Lesions}}
	\label{fig:2.Class6}
\end{figure}
...
antarkelas, di mana kelas mayoritas jauh lebih mendominasi kelas minoritas. 
Ketimpangan rasio dalam skala 100:1 hingga 10000:1 merupakan fenomena yang 
lumrah terjadi. Meskipun ketidakseimbangan data ini sering merujuk pada 
masalah biner, ketidakseimbangan juga dapat terjadi pada data 
\textit{multi-class} \cite{5128907}. \par
...
mengubah makna atau label dari data aslinya \cite{Goodfellow-et-al-2016}. Penggunaan teknik 
augmentasi data sebagai metode yang umum dan mudah untuk mengurangi 
\textit{overfitting} \cite{NIPS2012_c399862d}. \par

\subsubsection{\textit{Flip}} \label{II.Teori3.1}
...
berbagai sudut pandang \cite{Goodfellow-et-al-2016}. \par

\subsubsection{Rotasi} \label{II.Teori3.2}
...
dengan baik \cite{szeliski2022computer}. \par

\subsubsection{\textit{Random Erasing}} \label{II.Teori3.3}
...
\subsubsection{\textit{Random Augmentation}} \label{II.Teori3.4}
\textit{Random augmentation} merupakan teknik augmentasi yang dilakukan 
secara otomatis dengan tujuan untuk mengatasi kelemahan pada teknik 
sebelumnya seperti \textit{AutoAugment}. Masalah pada teknik sebelumnya 
dikarenakan membutuhkan komputasi yang tinggi untuk mencari augmentasi 
yang optimal dan membutuhkan \textit{search phase} yang rumit. Dengan 
\textit{random augmentation}, meringankan proses dengan cara menghapus 
\textit{search phase} \cite{cubuk2019randaugmentpracticalautomateddata}.

\subsubsection{\textit{Color Jitter}} \label{II.Teori3.5}
\textit{Color jitter} merupakan teknik augmentasi dengan cara mengubah 
warna pada gambar secara acak dengan tujuan membuat model tidak terlalu 
sensitif terhadap perubahan warna pada gambar. Teknik ini membuat model 
untuk menggeneralisasi menjadi lebih baik. Teknik ini membutuhkan empat 
komponen utama yaitu kecerahan, kontras, saturasi, dan \textit{Hue} \cite{zhang2023dive}.
...
\begin{figure}[H]
	\centering
	\fbox{\includegraphics[width=1.0\textwidth]{figure/cnnfig.jpg}}
	\caption{Illustrasi Convolutional Neural Network \cite{726791}}
	\label{fig:2.CnnArch}
\end{figure}
...
\begin{figure}[H] % Kalau menggunakan H, posisi gambar akan tepat dibawah teks
	\centering
	\fbox{\includegraphics[width=1.0\textwidth]{figure/resnet arch.jpg}}
	\caption{ResNet-34 dan CNN 34 \textit{layer} tanpa residual learning \cite{he2015deepresiduallearningimage}}
...
membandingkan performa keduanya. Pemilihan kedua arsitektur tersebut 
didasarkan pada perbedaan kompleksitas model dan kedalaman jaringan. 
Dengan membandingkan dua arsitektur yang memiliki jumlah parameter 
berbeda, diharapkan dapat diperoleh gambaran pengaruh kedalaman jaringan 
terhadap hasil klasifikasi.Berikut struktur dari kedua arsitektur dapat 
dilihat pada tabel \ref{table:resnet18} dan tabel \ref{table:resnet50}. \par
...
\begin{longtable}{|c|c|c|} % Longtable berguna agar tabel dapat terpotong di halaman baru
	\caption{Struktur ResNet-18 \cite{he2015deepresiduallearningimage}}
...
\setlength{\tabcolsep}{12pt} % Padding horizontal (default 6pt)
\begin{longtable}{|c|c|c|} % Longtable berguna agar tabel dapat terpotong di halaman baru
	\caption{Struktur ResNet-50 \cite{he2015deepresiduallearningimage}}
...
Pada tahun 2016, Kaiming He dkk memperkenalkan konsep \textit{residual learning}
pada arsitektur \textit{deep learning}. Lapisan-lapisan dalam jaringan disusun 
kembali untuk mempelajari fungsi residual yang mereferensikan input asli. 
Konsep \textit{residual learning} ini membuat lapisan-lapisan jaringan cukup 
mempelajari selisih atau residual dari input asli, yaitu $\mathcal{F}(x) := \mathcal{H}(x) - x$, 
dibandingkan mempelajari fungsi target langsung yaitu $\mathcal{H}(x)$. Hasil 
dari pembelajaran fungsi residual ini kemudian ditambahkan ke input asli ($x$) 
tersebut ($\mathcal{F}(x) + x$). Pengoptimalan fungsi residual lebih efisien 
dibandingkan pemetaan asli, karena jaringan cukup menekan nilai residual 
mendekati nol untuk mencapai pemetaan identitas. Hal ini diimplementasikan 
melalui \textit{shortcut connections} yang menjumlahkan input identitas 
langsung ke output lapisan. Konsep ini juga dapat mengatasi masalah degradasi 
yang disebabkan karena penambahan kedalaman pada jaringan \cite{he2015deepresiduallearningimage}. \par 

\subsubsection{Shortcut Connections} \label{II.Teori5.2}
\textit{Shortcut connections} merupakan jalur koneksi yang melompati satu atau 
lebih lapisan jaringan. Dalam arsitektur ResNet, jalur koneksi ini melakukan 
\textit{identity mapping}, yang mana \textit{output} dari lapisan yang berupa 
fungsi residual ditambahkan dengan input asli. Secara matematis 
direpresentasikan sebagai $y = \mathcal{F}(x, \{W_{i}\}) + x$, di mana $x$ 
adalah input dan $y$ adalah output. Penggunaan \textit{shortcut connections} 
memiliki keunggulan yaitu efisien. Dengan adanya jalur ini, tidak menambahkan 
parameter ekstra maupun meningkatkan kompleksitas komputasi pada jaringan \cite{he2015deepresiduallearningimage}. \par 
...
\begin{figure}[H] % Kalau menggunakan H, posisi gambar akan tepat dibawah teks
	\centering
	\fbox{\includegraphics[width=0.9\textwidth]{figure/confusionmatrix.png}}
...
Dari nilai komponen utama pada \textit{Confusion Matrix}, dapat dilakukan perhitungan kinerja model 
menggunakan persamaan berikut. \\

\begin{enumerate}[noitemsep]
	\item \textit{Accuracy} \\
	\textit{Accuracy} mengukur seberapa akurat model dalam mengklasifikasikan data. 
    Rumus akurasi dapat dilihat pada persamaan Rumus 2.1.
\end{enumerate}
\begin{equationcaptioned}[eq:2.accuracy]{
	Accuracy = \frac{TP + TN}{TP + TN + FP + FN}
}{
	Accuracy % Caption rumus
}
\end{equationcaptioned}

% \begin{equation}
% Accuracy = \frac{TP + TN}{TP + TN + FP + FN}
% \tag{Rumus 2.1 Accuracy}
% \end{equation}
...
\begin{equationcaptioned}[eq:2.precision]{
	...
	Precision % Caption rumus
}
\end{equationcaptioned}

% \begin{equation}
% Precision = \frac{TP}{TP + FP}
% \tag{Rumus 2.2 Precision}
% \end{equation}
...
\begin{enumerate}[noitemsep, resume]
    ...
    \textit{recall} dapat dilihat pada persamaan Rumus 2.3.
\end{enumerate}
\begin{equationcaptioned}[eq:2.recall]{
	Recall = \frac{TP}{TP + FN}
}{
	Recall % Caption rumus
}
\end{equationcaptioned}
...
\begin{enumerate}[noitemsep, resume]
    ...
    dapat dilihat pada persamaan Rumus 2.4.
\end{enumerate}
\begin{equationcaptioned}[eq:2.f1score]{
	F1-Score = 2 \times \frac{Precision \times Recall}{Precision + Recall}
}{
	F1-Score % Caption rumus
}
\end{equationcaptioned}

% \begin{equation}
% F1-Score = 2 \times \frac{Precision \times Recall}{Precision + Recall}
% \tag{Rumus 2.4 F1-Score}
% \end{equation}
...
klasifikasi biner maupun multi-kelas yang menghasilkan skor dalam interval [-1; +1], jika 
...
yang dapat dilihat pada Rumus 2.5.
